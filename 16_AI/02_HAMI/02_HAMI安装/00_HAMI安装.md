...
## 一、nvidia-gpu-operator安装toolkit

### 1、Values.yaml

>生产driver可以打开，他会给你的宿主机安装驱动，这里我选择用自己安装的。所以关闭。
...
validator:
  repository: registry.cn-shanghai.aliyuncs.com/gmqsbf
...
```bash
root@k8s-master-01:/kubeconfig/nvidia/gpu-operator# diff gpu-operator-default-values.yaml gpu-operator-values.yaml 
9c9
<   enabled: true
---
>   enabled: false
...
```bash
root@k8s-master-01:/kubeconfig/nvidia/gpu-operator# kubectl get pod -n gpu-operator 
NAME                                       READY   STATUS      RESTARTS   AGE
gpu-operator-658cf74dc5-5plg8              1/1     Running     0          28h
nvidia-container-toolkit-daemonset-gsb95   1/1     Running     0          28h
nvidia-cuda-validator-8lthx                0/1     Completed   0          28h
nvidia-operator-validator-4lsq2            1/1     Running     0          28h
root@k8s-master-01:/kubeconfig/nvidia/gpu-operator# kubectl get nodes -o custom-columns=NAME:.metadata.name,GPU:.status.allocatable."nvidia\.com/gpu"
NAME            GPU
k8s-master-01   <none>
k8s-node-01     <none>
k8s-node-02     <none>
...
```bash
helm install hami hami-charts/hami -n kube-system --set scheduler.kubeScheduler.imageTag=v1.33.1 --set devicePlugin.runtimeClassName=nvidia
```
...
helm install my-hami-webui hami-webui/hami-webui --set externalPrometheus.enabled=true --set externalPrometheus.address="http://prometheus-k8s.monitoring.svc:9090" -n kube-system
```
