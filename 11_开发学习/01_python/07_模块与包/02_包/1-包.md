...
>
>​    3、在当前执行文件所在的名称空间中得到一个名字pool，该名字指向`__init__`.py的名称空间，例如pool.xxx和pool.yyy中的xxx和yyy都是来自于pool下的`__init__`.py，也就是说导入包时并不会导入包下所有的子模块与子包
...
```python
## 针对pool.futures.process.ProcessPoolExecutor(3)，则需要
...
> ​    import也能使用绝对导入，导入过程中同样会依次执行包下的`__init__`.py,只是基于import导入的结果，使用时必须加上该前缀
...
> ​    在使用包时同样支持from pool.futures import * ，毫无疑问*代表的是futures下`__init__`.py中所有的名字，通用是用变量`__all__`来控制*代表的意思
...
>    最后说明一点，包内部的目录结构通常是包的开发者为了方便自己管理和维护代码而创建的，这种目录结构对包的使用者往往是无用的，此时通过操作`__init__.py`可以“隐藏”包内部的目录结构，降低使用难度，比如想要让使用者直接使用
