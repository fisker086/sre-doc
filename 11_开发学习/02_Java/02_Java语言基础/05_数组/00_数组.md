...
<img src="第1阶段：Java基本语法-第05章.png" alt="第1阶段：Java基本语法-第05章" style="zoom:45%;" />
...
> ​    现在需要统计某公司50个员工的工资情况，例如计算平均工资、找到最高工资等。用前面所学的知识，首先需要声明`50个变量`来分别记录每位员工的工资，这样做会显得很麻烦。因此我们可以使用容器进行操作，将所有的数据全部存储到一个容器中，统一管理。

#### **2.需求分析2：**

<img src="snipaste_20220317_000101.jpg" alt="snipaste_20220317_000101" style="zoom: 50%;" />
...
![image-20220317000952499](image-20220317000952499.png)
...
![image-20211221164709624](image-20211221164709624.png)
...
存储字母到letters数组之前：[ , , , , , , , , , , , , , , , , , , , , , , , , , ]
names数组的长度：5
存储姓名到names数组之前：[null,null,null,null,null]
*/
```
...
<img src="image-20220317004637748.png" alt="image-20220317004637748" style="zoom:80%;" />

案例3：抽奖程序的实现

制作一个抽奖程序，可以从控制台输入数名员工的姓名，然后随机抽取一名，作为年终大奖：马尔代夫7日游！
...
- 如果说可以把一维数组当成几何中的`线性图形`，那么二维数组就相当于是`一个表格`，像Excel中的表格、围棋棋盘一样。
  ...
  ![image-20220317004810263](image-20220317004810263.png)
...
```
  ...
  ![image-20220516095701345](image-20220516095701345.png)

  蓝框的几个元素，可以使用一维数组来存储。但现在发现每个元素下还有下拉框，其内部还有元素，那就需要使用二维数组来存储：

  ![image-20220516095829526](image-20220516095829526.png)
...
- 对于二维数组的理解，我们可以看成是一维数组array1又作为另一个一维数组array2的元素而存在。
  ...
  ![1561524724397](1561524724397-1647707344971.png)
...
![image-20220317005345385](image-20220317005345385.png)
...
一共有3组成绩.
第1组有4个学员，成绩如下：85	96	85	75	
第2组有5个学员，成绩如下：99	96	74	72	75	
第3组有4个学员，成绩如下：52	42	56	75	
*/
```
...
![image-20220317005436209](image-20220317005436209.png)
...
![image-20220317005549522](image-20220317005549522.png)
...
![1574577970893](1574577970893.png)
...
<img src="snipaste_20220317_225346.jpg" alt="snipaste_20220317_225346" style="zoom:80%;" />![snipaste_20220317_225359](snipaste_20220317_225359.jpg)
...
![1659199523932](1659199523932.png)
...
某个政府机构使用传真传递数据信息，数据是8位的整数。为了确保数据安全，在传递过程中需要加密。加密规则如下：
首先，将各个位上的数据倒序，然后将每位数字都加上6，再用和除以8的余数代替该数字，最后交换首末位上的数字。 

请任意给定一个8位的整数，并将加密后的结果在控制台打印出来。
...
![image-20220317230955644](image-20220317230955644.png)

<img src="image-20220623210601915.png" alt="image-20220623210601915" style="zoom:67%;" />
...
- `时间复杂度`：分析关键字的比较次数和记录的移动次数
  - 常见的算法时间复杂度由小到大依次为：Ο(1)＜Ο(log2n)＜Ο(n)＜Ο(nlog2n)＜Ο(n<sup>2</sup>)＜Ο(n<sup>3</sup>)＜…＜Ο(2<sup>n</sup>)＜Ο(n!)
    	...
    	并且一个算法花费的时间与算法中语句的执行次数成正比例，哪个算法中语句执行次数多，它花费时间就多。一个算法中的语句执行次数称为语句频度或时间频度。记为T(n)。n称为问题的规模，当n不断变化时，时间频度T(n)也会不断变化。但有时我们想知道它变化时呈现什么规律。为此，我们引入时间复杂度概念。 
  ...
  - `稳定性`：若两个记录A和B的关键字值相等，但排序后A、B的先后次序保持不变，则称这种排序算法是稳定的。
    ...
    ![image-20211222113701365](image-20211222113701365.png)
...
![image-20211222111142684](image-20211222111142684.png)
...
![image-20220824003440106](image-20220824003440106.png)

#### 3.冒泡排序

**介绍：**

Java中的经典算法之冒泡排序（Bubble Sort）。冒泡排序的原理非常简单，它重复地走访过要排序的数列，一次比较两个元素，如果他们的顺序不合适就把他们交换过来。

<img src="image-20220516094637228.png" alt="image-20220516094637228" style="zoom:67%;" />
...
快速排序（Quick Sort）由`图灵奖`获得者`Tony Hoare`发明，被列为`20世纪十大算法之一`，是迄今为止所有内排序算法中速度最快的一种。冒泡排序的升级版，交换排序的一种。快速排序的时间复杂度为O(nlog(n))。
...
![image-20220317235859754](image-20220317235859754.png)

![image-20220317235908152](image-20220317235908152.png)

图示2：

![image-20220317235922776](image-20220317235922776.png)
...
![](空指针异常-1647708157677.jpg)

**空指针异常在内存图中的表现**

![1572338767825](1572338767825-1647708157678.png)
