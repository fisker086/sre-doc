...
>
>- 流程控制方式采用结构化程序设计中规定的三种`基本流程结构`，即：
>  - **顺序结构**
...
>    - 注：JDK5.0 提供了`foreach`循环，方便的遍历集合、数组元素。
...
![image-20211218093256771](./image-20211218093256771.png)
...
![image-20220514160139926](./image-20220514160139926-1705238241053-3.png)
...
![image-20220514160243421](./image-20220514160243421.png)
...
![image-20220514160538651](./image-20220514160538651.png)
...
![image-20221027185234087](./image-20221027185234087.png)

![image-20221027185500389](./image-20221027185500389.png)
...
**练习2：**小明参加期末Java考试，通过考试成绩，判断其Java等级，成绩范围[0,100]
...
![image-20220514101841710](./image-20220514101841710.png)
...
**案例4：**编写程序：从键盘上输入2022年的“month”和“day”，要求通过程序输出输入的日期为2022年的第几天。
...
class SwitchCaseTest4 {
	public static void main(String[] args) {
		...
		*/
		...
		System.out.println(month + "月" + day + "日是2022年的第" + sumDays + "天");
        //关闭资源
		scan.close();
	}
}
...
![image-20220314005350344](./image-20220314005350344.png)
...
![image-20220315013023236](./image-20220315013023236.png)
...
>举例：遍历1-100以内的偶数，并获取偶数的个数，获取所有的偶数的和
...
![image-20220315013023236](./image-20220315013023236-1705241345373-12.png)
...
**练习1：**从键盘输入整数，输入0结束，统计输入的正数、负数的个数。
...
![image-20220512165558698](./image-20220512165558698.png)
...
> - **所谓嵌套循环**，是指一个循环A的循环体是另一个循环B。比如，for循环里面还有一个for循环，就是嵌套循环。其中，for ,while ,do…while均可以作为外层循环或内层循环。
>   - 外层循环：循环A
>   - 内层循环：循环B
...
![1561447396381](./1561447396381-1705242436751-18.png)
...
>- 如何从键盘获取不同类型（8种基本数据类型、String类型）的变量：使用Scanner类。
>
>- 键盘输入代码的四个步骤：
>  1. 导包：`import java.util.Scanner;`
>  2. 创建Scanner类型的对象：`Scanner scan = new Scanner(System.in);`
>  3. 调用Scanner类的相关方法（`next() / nextXxx()`），来获取指定类型的变量
>  4. 释放资源，归还资源：`scan.close();`
>- 注意：需要根据相应的方法，来输入指定类型的值。如果输入的数据类型与要求的类型不匹配时，会报`异常 InputMisMatchException`导致程序终止。
...
public class ScannerTest1 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("欢迎管理你好我好交友网站！");
...
public class ScannerTest2 {
    public static void main(String[] args) {
        ...
        //String name = input.next();//张 三  只能接收张，后面的空格和三无法接收
        String name = input.nextLine(); //张 三 可以接收完整的数据
...
class ScannerExer1 {
	public static void main(String[] args) {
		...
		*/
		...
		//判断
		if(height >= 180 && wealth >= 1.0 && "是".equals(isHandsome)){  //知识点：判断两个字符串是否相等，使用String的equals()
			System.out.println("我一定要嫁给他!!!");
